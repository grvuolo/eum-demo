...
function postBeacon(beacon, user) {
    ...
    for (let destination in config.reporting) {
        ...
        logger.info({
            ...
            url: config.reporting[destination].url
        });
        beacon.k = config.reporting[destination].apiKey;
        const body = lineEncode(beacon);
        let req = https.request(config.reporting[destination].url, options, (res) => {
            logger.info({
                msg: 'beacon sent',
                destination: destination,
...
function generateErrorBeacon(page, user, pl, sid) {
    ...
    beacon.st = page.error.text + ' at ' + beacon.l;
    beacon.p = page.title;
